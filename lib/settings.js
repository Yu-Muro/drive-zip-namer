...
export function parseImport(text) {
  ...
  if (data.version !== 1 && data.version !== EXPORT_VERSION) {
    throw new Error(`未対応の設定ファイルバージョンです: ${String(data.version)}`);
  }
...
function validateImportShape(data) {
  if (!data.userSettings || typeof data.userSettings !== "object") {
    throw new Error("設定項目が見つかりません。");
  }
  if (!Array.isArray(data.presets)) {
    throw new Error("プリセットの形式が正しくありません。");
  }
  if (data.presets.length > MAX_PRESETS) {
    throw new Error(`プリセットは${MAX_PRESETS}件までです。`);
  }
  ...
  for (const preset of data.presets) {
    if (
      ...
      throw new Error("プリセットに不正な項目があります。");
    }
  }
}
